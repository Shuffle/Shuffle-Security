/**
 * SearchDialog — Unified Command Palette search component for Shuffle.
 *
 * Shared between Shuffle Automation (shaffuru / shuffler.io) and Shuffle Security (shuffle.security).
 *
 * Features:
 ...
 * - Strict Shuffle branding: No emojis, clean engineering typography, HSL design tokens.
 */
...
export interface NavResult {
  ...
  group: 'Pages' | 'Security' | 'Automation';
  platform: 'automation' | 'security' | 'both';
  keywords?: string[];
  hiddenUnlessSearched?: boolean;
}

export interface OrgWorkflowResult {
  type: 'org_workflow';
  workflow: WorkflowSummary;
}

export interface PublicWorkflowResult {
  type: 'public_workflow';
  workflow: {
    id: string;
    name: string;
    description?: string;
...
export interface SearchDialogProps {
  ...
  hostPlatform?: 'automation' | 'security' | 'auto';
...
export const SearchDialog = ({
  ...
  const algoliaDebounceRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  const corrDebounceRef = useRef<ReturnType<typeof setTimeout> | null>(null);

  // Fetch and cache local tenant workflows (non-blocking in-memory cache)
  ...
  const currentPlatform: 'automation' | 'security' = useMemo(() => {
    if (hostPlatform && hostPlatform !== 'auto') return hostPlatform;
    if (typeof window !== 'undefined') {
      const host = window.location.hostname.toLowerCase();
      if (host === 'shuffle.security' || host.endsWith('.shuffle.security')) {
        return 'security';
      }
      if (host === 'shuffler.io' || host.endsWith('.shuffler.io')) {
        return 'automation';
      }
    }
    return 'security';
  ...
  const matchedOrgWorkflows = useMemo((): OrgWorkflowResult[] => {
  ...
  // Direct matches vs other correlations
  const qLower = query.trim().toLowerCase();
  const directMatchCorrelations = useMemo(() => {
    if (!qLower) return [];
    return correlationResults.filter((c) => c.key.toLowerCase() === qLower);
  }, [qLower, correlationResults]);

  const otherCorrelations = useMemo(() => {
    return correlationResults.filter((c) => !directMatchCorrelations.includes(c));
  }, [correlationResults, directMatchCorrelations]);
  ...
  const searchCorrelations = useCallback(
    async (searchQuery: string) => {
      const q = searchQuery.trim();
      if (!enableCorrelations || !q || q.length < 2) {
        setCorrelationResults([]);
  ...
  const handleSelect = useCallback(
    (result: SearchResult) => {
      ...
      if (result.type === 'nav') {
        ...
        if (targetPlatform === 'both' || targetPlatform === currentPlatform) {
          safeNavigate(result.path);
        } else if (targetPlatform === 'automation') {
          navigateToShuffleCore(result.path, { newTab: true });
        } else if (targetPlatform === 'security') {
        ...
        safeNavigate(result.doc.path || `/docs/${result.doc.slug}`);
        ...
        if (key) {
          if (currentPlatform === 'security') {
            safeNavigate(`/incidents/${key}`);
          } else {
            navigateToShuffleSecurity(`/incidents/${key}`, { newTab: true });
          }
        }
      } else if (result.type === 'see_all') {
        safeNavigate(`/search?q=${encodeURIComponent(result.query)}`);
      }
    },
    [currentPlatform, handleOpenChange, safeNavigate],
  );
  ...
  return (
    ...
    >
      <DialogContent sx={{ p: 0, display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
        ...
        >
          <SearchIcon size={18} style={{ color: 'hsl(var(--muted-foreground))', flexShrink: 0 }} />
          <InputBase
            inputRef={inputRef}
            value={query}
          ...
          />
          {isAnyLoading && (
            <CircularProgress size={16} sx={{ color: 'hsl(var(--primary))', flexShrink: 0 }} />
          )}
          <Box
            sx={{
              px: 0.75,
          ...
          {results.map((item, idx) => {
            ...
            if (item.type === 'nav') {
              const isCrossPlatform =
                item.platform !== 'both' && item.platform !== currentPlatform;
              return (
                <Box
                ...
                >
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, minWidth: 0 }}>
                    <Box sx={{ color: 'hsl(var(--muted-foreground))', display: 'flex', alignItems: 'center' }}>
                      {item.icon}
                    </Box>
                    <Typography sx={{ fontSize: 13, fontWeight: 500, color: 'hsl(var(--foreground))' }}>
                      {item.label}
                    </Typography>
                  </Box>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                    {isCrossPlatform && (
                      ...
                      >
                        {item.platform === 'security' ? 'Security' : 'Automation'}
            ...
            if (item.type === 'org_workflow') {
              return (
                ...
                >
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, minWidth: 0 }}>
                    <Workflow size={16} style={{ color: 'hsl(var(--primary))', flexShrink: 0 }} />
                    <Box sx={{ minWidth: 0 }}>
                      <Typography sx={{ fontSize: 13, fontWeight: 500, color: 'hsl(var(--foreground))', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                        {item.workflow.name}
                      </Typography>
                      {item.workflow.description && (
                        <Typography variant="caption" sx={{ color: 'hsl(var(--muted-foreground))', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', display: 'block' }}>
                          {item.workflow.description}
                        </Typography>
                      )}
                    </Box>
            ...
            if (item.type === 'public_workflow') {
              return (
                ...
                >
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, minWidth: 0 }}>
                    <Compass size={16} style={{ color: 'hsl(var(--muted-foreground))', flexShrink: 0 }} />
                    <Box sx={{ minWidth: 0 }}>
                      <Typography sx={{ fontSize: 13, fontWeight: 500, color: 'hsl(var(--foreground))', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                        {item.workflow.name}
                      </Typography>
                ...
                >
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, minWidth: 0 }}>
                    {item.app.image_url ? (
                      ...
                      <Braces size={16} style={{ color: 'hsl(var(--muted-foreground))', flexShrink: 0 }} />
                    )}
                    <Box sx={{ minWidth: 0 }}>
                      <Typography sx={{ fontSize: 13, fontWeight: 500, color: 'hsl(var(--foreground))', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                        {item.app.name}
            ...
            if (item.type === 'doc') {
              return (
                ...
                >
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, minWidth: 0 }}>
                    <BookOpen size={16} style={{ color: 'hsl(var(--muted-foreground))', flexShrink: 0 }} />
                    <Box sx={{ minWidth: 0 }}>
                      <Typography sx={{ fontSize: 13, fontWeight: 500, color: 'hsl(var(--foreground))', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                        {item.doc.label}
                      </Typography>
            ...
            if (item.type === 'correlation') {
              return (
                ...
                >
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, minWidth: 0 }}>
                    <Shield size={16} style={{ color: 'hsl(var(--primary))', flexShrink: 0 }} />
                    <Box sx={{ minWidth: 0 }}>
                      <Typography sx={{ fontSize: 13, fontWeight: 500, color: 'hsl(var(--foreground))', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', fontFamily: 'monospace' }}>
              ...
              return (
                ...
                >
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                    <SearchIcon size={14} style={{ color: 'hsl(var(--primary))' }} />
                    <Typography sx={{ fontSize: 13, fontWeight: 600, color: 'hsl(var(--primary))' }}>
        ...
        >
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
              <Box sx={{ px: 0.5, py: 0.1, borderRadius: '3px', border: '1px solid hsl(var(--border))', fontSize: 10, fontFamily: 'monospace' }}>↑</Box>
              <Box sx={{ px: 0.5, py: 0.1, borderRadius: '3px', border: '1px solid hsl(var(--border))', fontSize: 10, fontFamily: 'monospace' }}>↓</Box>
              <Typography variant="caption" sx={{ fontSize: 11, color: 'hsl(var(--muted-foreground))' }}>Navigate</Typography>
            </Box>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
              <Box sx={{ px: 0.5, py: 0.1, borderRadius: '3px', border: '1px solid hsl(var(--border))', fontSize: 10, fontFamily: 'monospace' }}>↵</Box>
              <Typography variant="caption" sx={{ fontSize: 11, color: 'hsl(var(--muted-foreground))' }}>Select</Typography>
            </Box>
          </Box>
          <Typography variant="caption" sx={{ fontSize: 11, color: 'hsl(var(--muted-foreground))' }}>
